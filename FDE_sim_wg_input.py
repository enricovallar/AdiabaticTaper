...
Modules:
    - waveguides_simulations: Contains GeomBuilder and WaveguideModeFinder classes for building geometries and finding waveguide modes.
    - importlib.util: Used for dynamic import of the Lumerical API.
...
Constants:
    ...
    - PATH_DATA: Path to save the simulation data.
    - DATA_FILE_NAME: Name of the file to save the simulation data.
Variables:
    - height_InP: Height of the InP layer.
    - height_SiN: Height of the SiN layer.
    - width_start, width_stop, width_step: Parameters to define the range and step size for waveguide widths.
    - width_array: Array of waveguide widths.
    - height_start, height_end, height_step: Parameters to define the range and step size for waveguide heights.
    - height_array: Array of waveguide heights.
    - data_points: List to store the simulation results.
Functions:
    - None
Workflow:
    1. Set up paths and constants.
    2. Create directories if they do not exist.
    3. Define waveguide dimensions.
    4. Loop through each combination of width and height to perform simulations.
    5. Save the simulation results.
    6. Load the saved simulation data.
    7. Analyze the loaded data to extract relevant information.
    8. Plot the results and save the plots.
Usage:
    Run the script to perform waveguide simulations, save the results, and generate plots for analysis.
"""
...
# #  DEPRECATED: run  this if you forget to save some data
